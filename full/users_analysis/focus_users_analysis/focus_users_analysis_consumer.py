...
def focus_users_analysis():
    ...
    focus_users_analysis = user_data.select("login", "followers", "following")
    
    query_console = focus_users_analysis.writeStream \
                    .outputMode("append") \
                    .format("console") \
                    .start()

    hdfs_path = "hdfs://master:9000/output/focus_users_analysis"
    ck_path = "hdfs://master:9000/checkpoint/focus_users_analysis"

    query_hdfs = focus_users_analysis.writeStream \
                        .outputMode("append") \
                        .format("parquet") \
    ...
    es_index = "users_focus"

    query_es = focus_users_analysis.writeStream \
                    .outputMode("append") \
                    ...
                    .option("checkpointLocation", "hdfs://master:9000/checkpoint/elastic/focus_users_analysis") \
                    .start()
